...
def init_application(
    ...
    persistence: bool = typer.Option(
        False, "--persist", "-p", help="Enable database persistence"
    ),
    ...
    if reload and workers > 1:
        typer.secho(
            "Warning: 'reload' is incompatible with multiple workers. Using a single worker.",
...
def run_application(
    ...
    uvicorn.run(
        app, host=host, port=port, reload=reload, workers=workers if not reload else 1
    )
